...
//
// NOTE: Currently XFAIL'd. The driving issue is upstream of these passes:
// when the IModule is constructed via the `luthier-mock-inject-at-function-entry`
// IR pass (hook-stub source), the legacy MIR PM's addISelPasses() chain runs
// the IR-stage codegen-prep passes but does not actually lower the payload
// MachineFunction (empty body in the printed MIR). The two new passes are
// wired in, registered, and run without crashing — but they have no MIR to
// transform until that ISel lowering is fixed.
//
// XFAIL: *
//
...
// CHECK:      name:            luthier.payload.vector_add
// CHECK:      body:
// CHECK:      bb.0
// CHECK:      %{{[0-9]+}}:{{[a-z_0-9]+}} = COPY $
// CHECK:      $
// CHECK-SAME: = COPY %{{[0-9]+}}
