...
AMDGPUMockLoaderPrinter::run(llvm::Module &M,
  ...
  for (const auto &LCO : Loader.loaded_code_objects()) {
    ...
    for (const auto PH : LCO.getLoadSegments()) {
      constexpr auto Fmt = "0x%016" PRIx64 " ";
      OS << "\tType: ";
      printELFType(PH.get(), OS);
      OS << "\n";
      OS << "\tOffset: "
         << llvm::format(Fmt, static_cast<uint64_t>(PH.get().p_offset)) << "\n";
      OS << "\tVAddr: "
         << llvm::format(Fmt, static_cast<uint64_t>(PH.get().p_vaddr)) << "\n";
      OS << "PAddr: "
         << llvm::format(Fmt, static_cast<uint64_t>(PH.get().p_paddr)) << "\n";
      OS << llvm::format("align 2**%u\n",
                         llvm::countr_zero<uint64_t>(PH.get().p_align))
         << "\n";
      OS << "Filesz: "
         << llvm::format(Fmt, static_cast<uint64_t>(PH.get().p_filesz)) << "\n";
      OS << "Memsz: "
         << llvm::format(Fmt, static_cast<uint64_t>(PH.get().p_memsz)) << "\n";
      OS << "Flags: " << ((PH.get().p_flags & llvm::ELF::PF_R) ? "r" : "-")
         << ((PH.get().p_flags & llvm::ELF::PF_W) ? "w" : "-")
         << ((PH.get().p_flags & llvm::ELF::PF_X) ? "x" : "-") << "\n";

      if (PH.get().p_flags & llvm::ELF::PF_X) {
        /// Disassemble both the loaded segmet
        uint64_t SectionCurrAddr =
            reinterpret_cast<uint64_t>(
                LCO.getCodeObject().getMemoryBufferRef().getBufferStart()) +
            PH.get().p_offset;
        uint64_t SegmentCurrAddr =
            reinterpret_cast<uint64_t>(LCO.getLoadedRegion().data()) +
            PH.get().p_vaddr;
        uint64_t SegmentEndAddr = SegmentCurrAddr + PH.get().p_filesz;
